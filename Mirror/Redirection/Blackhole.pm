...
sub should_blackhole {
	...
	return 1 if ($url =~ m,^dists/[^/]+/(?:main|contrib|non-free)/binary-[^/]+/Packages$,);
	return 1 if ($url =~ m,^dists/sid, && (
	...
	return 1 if ($url =~ m,^dists/squeeze-lts/(?:main|contrib|non-free)/binary-(?!i386|amd64),);
